---
title: "Walt_Georgia_Data_Wrangling1"
...
```{r}
unique(fruits) ##takes a vector and returns only unique elements of it (3 fruits)
```
...
## You can string things together, such as unique objects in dataframe, then finding the amount (length) of those objects
```
...
```{r}
# Import the data
cereal <- read_csv(here("cereal.csv")) ##USe read_csv to bring in a tibble format (not a regular dataframe)
...
# A new tibble with only these three selected columns 
```
...
```{r}
cereal_fiber <- cereal %>% 
  select(name, calories, fiber) ##Create a new object to save selected tibble columns in a new dataframe
```
...
```{r include = TRUE}
cereal %>% 
  select(-name, -mfr) ##Use negative symbol to exclude 
```
...
##We created a new tibble called cereal_sugar that includes name, manufacturer, and sugar content
```
...
```{r}
cereal_sugar %>% 
  # Rename the mfr column to a more informative manufacturer (note new name = old name)
...
cereal_sugar %>% 
  rename(manufacturer = mfr,
         cereal_name = name) #Rename another column with comma after the first row
```
...
```{r}
cereal_sugar %>% 
  relocate(mfr, .before = name) #Moving "manufacturer before "name" columns using relocate
```
...
```{r}
cereal_sugar %>% 
  filter(sugars > 12) ##Use filter (column > vlaue) to look at greater than
```
...
# Use == to say "exactly" and use quotes since it's a character --> showing 23 cereals from Kelloggs
```
...
cereal_sugar %>% 
  filter (sugars > 12,
          mfr == "Kelloggs") 
## Looked at Kellogg cerals with >12 grams of sugar and from Kelloggs, which are 4 cereals
```
...
cereal %>% 
  ...
  filter(potass < 30)
## We find that 8 cereals have potassium that is less than 30 (**note potassium was incorrectly spelled, but we looked at colnames)
```
...
```{r}
cereal %>% 
  filter(calories > 120) %>% ##Look at all cereals with >120 calories
  select (name, fiber, fat, sodium) ##Pull out those cereals for its name, fiber, fat, and sodium
## In total, there are 8 cereals that have these parameters
```
...
```{r}
cereal %>% 
  arrange(calories) ##Sorts all cereals from lowest to highest calories (default for arrange function)
```

```{r}
cereal %>% 
  # The desc() reverses the order, making it highest to lowest
  arrange(desc(calories)) ##use arrange(desc(colname)) to look at descending order
```
...
# First ordered by manufacturer (alphabetical order), and then in ascending order by calories based on those manufacturers
```
...
cereal_carbs %>% 
  ...
  mutate(sugars_total = sugars*2) ##Calculating two servings of sugar in a new column (sugar*2) -- note multiplication is *
...
```{r}
cereal_carbs %>%  ##Overrite original column called sugars using the mutate function
...
```{r}
cereal_carbs %>% 
  mutate(sugars_with_milk = sugars + 5) ##Adding in the sugar from milk
```
...
```{r}
cereal_carbs %>% 
  mutate(total_carbs = carbo + sugars) %>% ##Use pipeline to make another column using the mutate function
...
cereal_carbs %>% 
  mutate(total_carbs = carbo + sugars,
         sugars_with_milk = sugars + 5) ##I'm glad the comma still works to separate two rows in the same function
```
...
bison %>% 
  mutate (age = rec_year-animal_yob,
          weight_kg = animal_weight*0.453592)
```
